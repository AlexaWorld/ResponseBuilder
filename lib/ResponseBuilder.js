...
class ResponseBuilder {
	...
	SaySsml(ssmlBuilder) {
		...
		if (isTypeOf(ssmlBuilder, 'SsmlBuilder')) {
			this.response.outputSpeech = ssmlBuilder.Build();
			return this;
		}
	...
	RepromptSsml(ssmlBuilder) {
		...
		// String aa SSML
		if (isTypeOf(ssmlBuilder, 'String')) {
			this.response.reprompt = createSsml(ssmlBuilder);
			return this;
		}

		// Already a SSML objekt
		if (isTypeOf(ssmlBuilder, 'Object')) {
			this.response.reprompt = ssmlBuilder;
			return this;
		}

		// A SsmlBuilder
		if (isTypeOf(ssmlBuilder, 'SsmlBuilder')) {
			this.response.reprompt = ssmlBuilder.Build();
			return this;
		}
		...
		var ssml = new SsmlBuilder();
		ssmlBuilder(ssml);
		this.response.reprompt = ssml.Build();
		this.shouldEndSession = false;
		return this;
