...
class SsmlBuilder {
	...
	Rate(text, rate) {
		this.textToSpeak.push(this.Prosody("rate", rate, text));
		return this;
	}
	...
	Pitch(text, pitch) {
		this.textToSpeak.push(this.Prosody("pitch", pitch, text));
		return this;
	}
	...
	WithVolume(text, volume) {
		this.textToSpeak.push(this.Prosody("volume", volume, text));
		return this;
	}
	...
	SayAs(text, interpreter) {
		var interpretAs = interpreter; // interpreter.ToString().Replace("_", "-");
